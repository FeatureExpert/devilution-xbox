	...
	IN RESADDITIONAL *pAdditional)
{
	...
	while ((pName = *ppName) != NULL) {
		if (fNameID) {
			if (Name->Ordinal == pName->Name->Ordinal && pNameMatch == NULL) {
				pNameMatch = pName;
				break;
			}
			...
			if (wcscmp(Name->szStr, pName->Name->szStr) == 0 && pNameMatch == NULL) {
				pNameMatch = pName;
				break;
			}
...

{
	...
	while (offHere < cbInFile) {
		...
		//
		MyRead(fhIn, (PUCHAR)&pAdditional->DataVersion, sizeof(RESADDITIONAL) - 2 * sizeof(ULONG));

		//
		// if name table then discard it
		//

		if (Type->discriminant == IS_STRING || !(Type->Ordinal == (INT)RT_DLGINCLUDE)) {
			if (fVerbose) {
				if (Type->discriminant == IS_STRING) {
	...
	offNameDirs    = offTypeDir + (sizeof(IMAGE_RESOURCE_DIRECTORY_ENTRY) * NumberOfTypes);
	offLangDirs    = offNameDirs + (sizeof(IMAGE_RESOURCE_DIRECTORY) * NumberOfTypes) + (sizeof(IMAGE_RESOURCE_DIRECTORY_ENTRY) * NumberOfNames);
	offDataEntries = offLangDirs + (sizeof(IMAGE_RESOURCE_DIRECTORY) * NumberOfLangDirs) + (sizeof(IMAGE_RESOURCE_DIRECTORY_ENTRY) * NumberOfLanguages);

	offStringTable = offDataEntries + (sizeof(IMAGE_RESOURCE_DATA_ENTRY) * NumberOfResources);
...
#endif /* _DEBUG */
	InitializeDir(ResourceTypeDirectory, 0, 0, timeDate, NumberOfTypesName, NumberOfTypesID);
	...
	while (pType) {
		ResourceTypeDirectoryEntry->Name         = IMAGE_RESOURCE_NAME_IS_STRING | (offStringTable + pType->Type->OffsetToString);
		ResourceTypeDirectoryEntry->OffsetToData = IMAGE_RESOURCE_DATA_IS_DIRECTORY | (ULONG)((PUCHAR)ResourceNameDirectory - ResTable);
...
#endif /* _DEBUG */
		InitializeDir(ResourceNameDirectory, 0, 0, timeDate, pType->NumberOfNamesName, pType->NumberOfNamesID);

		ResourceNameDirectoryEntry = (PIMAGE_RESOURCE_DIRECTORY_ENTRY)(ResourceNameDirectory + 1);
		...
		while (pName) {
...
#endif /* _DEBUG */
			InitializeDir(ResourceLangDirectory, 0, 0, timeDate, 0, pName->NumberOfLanguages);
			ResourceLangDirectoryEntry = (PIMAGE_RESOURCE_DIRECTORY_ENTRY)(ResourceLangDirectory + 1);

			ResourceNameDirectoryEntry->Name         = IMAGE_RESOURCE_NAME_IS_STRING | (offStringTable + pName->Name->OffsetToString);
			ResourceNameDirectoryEntry->OffsetToData = IMAGE_RESOURCE_DATA_IS_DIRECTORY | (ULONG)((PUCHAR)ResourceLangDirectory - ResTable);

			cLanguages = pName->NumberOfLanguages;
			while (cLanguages--) {
				ResourceLangDirectoryEntry->Name         = (ULONG)pName->pAdditional->LanguageId;
				ResourceLangDirectoryEntry->OffsetToData = (ULONG)((PUCHAR)ResourceDataEntry - ResTable);
				pName->OffsetToDataEntry                 = ResourceLangDirectoryEntry->OffsetToData;
			...
			ResourceLangDirectory = (PIMAGE_RESOURCE_DIRECTORY)ResourceLangDirectoryEntry;
	...
	while (pstring) {
		...
		ResourceStringEntry = (PIMAGE_RESOURCE_DIRECTORY_STRING)((PUCHAR)ResourceStringEntry + pstring->cbData);
	...
	MySeek(fhOut, cbResTable, SEEK_SET);
	...
	while (pName) {
		...
		ResourceDataEntry               = (PIMAGE_RESOURCE_DATA_ENTRY)(ResTable + pName->OffsetToDataEntry);
